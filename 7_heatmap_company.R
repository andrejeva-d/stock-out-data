...
dat<-data.frame(TABLE_month$n, TABLE_month$Company, TABLE_month$dates_mon)
colnames(dat)<-c("n", "Company", "date")
dat<-(dat) %>% 
  group_by(Company, date) %>% 
  summarise(Frequency = sum(n))
dat<-dcast(dat, Company ~ date)
dat[is.na(dat)]<-0
...
count_uniq<-unique(TABLE_SALES[,c("Nordic_number", "Company")]) %>% 
  count(Company) 
keep<-count_uniq$n>5
...
dat_norm<-merge(dat, count_uniq, by.x="Company", by.y="Company")
...
if (country == "FI") {
  keep<-(rowMeans(dat_norm2, na.rm=TRUE)>0.5)
  dat_norm2<-dat_norm2[keep,]
}
if (country == "SE") {
  keep<-(rowMeans(dat_norm2, na.rm=TRUE)>0)
